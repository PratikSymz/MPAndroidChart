 ...
 */
public abstract class LineScatterCandleRadarDataSet<T extends Entry> extends
    BarLineScatterCandleBubbleDataSet<T> implements ILineScatterCandleRadarDataSet<T> {
  ...
  /**
   * the radii and colors of the selected highlights
   */
  ...
  /**
   * Enables / disables the circular highlight-indicator. If disabled, the indicator is not drawn. default: false
   *
   * @param enabled
   */
  ...
  public void setupCircularHighlightIndicator(float[] circleRadii, int[] circleColors) {
    if (circleRadii == null || circleColors == null
      ...
      Log.e(getClass().getSimpleName(), "You must specify at least one circle radius and at" +
          " least one circle color.");
      return;
    }
